...
namespace TransCarga
{
    class impDV
    {
        ...
        private void imprime_TK(object sender, PrintPageEventArgs e)
        {
            ...
            Image photo = Image.FromFile(va[0]); // Image.FromFile(logoclt);
            for (int i = 1; i <= copias; i++)
            {
                ...
                recdom = new RectangleF(puntoF, cuad);
                e.Graphics.DrawString(vs[6].Trim(), lt_peq, Brushes.Black, recdom, StringFormat.GenericTypographic);                  // DNI/RUC cliente
                ...
                string codigo = rucclie + separ + vs[21] + separ +
                    ...
                    vs[14] + separ + vs[15] + separ +
                    vs[5].Substring(6, 4) + "-" + vs[5].Substring(3, 2) + "-" + vs[5].Substring(0, 2) + separ + vs[22] + separ +
                    vs[7] + separ;  // string.Format("{0:yyyy-MM-dd}", tx_fechope.Text)
                                                //
